...
class AllSprites(pygame.sprite.Group):
    ...
    def custom_draw(self,player):
        ...
        for x in range(self.sky_num):
            ...
            self.display_surface.blit(self.bg_sky,(x_pos - self.offset.x / 2.5, 100 - self.offset.y / 2.5))

        for x in range(self.sky_num2):
            x_pos = -self.padding + (x * self.sky_width2)
            self.display_surface.blit(self.fg_sky, (x_pos - self.offset.x / 2, 750 - self.offset.y / 2))
...
class Main:
    ...
    def setup(self):
        ...
        # tiles ->  BG, BG Detail, FG Detail Bottom, FG Detail Top
        for layers in ['s3', 's2', 's1','BG', 'BG Detail', 'FG Detail Bottom', 'FG Detail Top',]:
        ...
        for platform in self.platform_sprites.sprites():
            for border in self.platform_border_rects:
                if platform.rect.colliderect(border):
                    if platform.direction.y < 0: #platform going up
                        platform.rect.top = border.bottom
                        platform.pos.y = platform.rect.y
                        platform.direction.y = 1
                    else:   #platform going down
    ...
    def run(self):
        while True:
            ...
            self.display_surface.fill((7,12,113))
